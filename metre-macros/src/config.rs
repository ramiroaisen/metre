...
fn ty_is_option(ty: &syn::Type) -> bool {
  ...
  match extract_type_path(ty) {
    None => false,
    Some(path) => {
      extract_option_segment(path).is_some()
    }
  }
}
...
pub fn config(input: DeriveInput) -> Result<TokenStream, syn::Error> {
  ...
  let partial_name = syn::Ident::new(&format!("Partial{}", name), Span::call_site());
  let vis = &input.vis;
  ...
  let container_env_prefix_fmt: LitStr = container_attrs
    ...
    .unwrap_or_else(|| LitStr::new("{prefix}", Span::call_site()));

  let get_container_env_prefix = if fmt_has_prefix(&container_env_prefix_fmt.value()) {
    quote! { format!(#container_env_prefix_fmt, prefix=env_prefix) }
  } else {
  ...
  for field in &fields.named {
    ...
    let env_fmt: LitStr = attrs
      ...
      .unwrap_or_else(|| {
        if attrs.flatten {
          LitStr::new("{prefix}", span)
        } else {
          LitStr::new(&format!("{{prefix}}{}", env_name), span)
        }
      });

    let get_field_env_key = if fmt_has_prefix(&env_fmt.value()) {
      span_quote! { format!(#env_fmt, prefix=container_env_prefix) }
    } else {
